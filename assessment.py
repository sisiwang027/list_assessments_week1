...
def print_indices(items):
    ...
    """
    for index_item, value in enumerate(items):
        print index_item, value
...
def every_other_item(items):
    ...
    return [item for item_index, item in enumerate(items) if item_index % 2 == 0]
...
def largest_n_items(items, n):
    ...
    #     return None
    # else:
    #     return [item if item_index <= n-1 else None for item_index, item in enumerate(items.sort(revers = True))].sort()
    items.sort()
    if n == 0:
        return []
    else:
